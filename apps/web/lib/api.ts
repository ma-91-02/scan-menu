...
const fallbackMenu: Array<
  MenuItem & { displayName: string; displayDescription: string }
> = [
  {
    id: "mi_salmon_bowl",
    restaurantId: "rst_bistro_01",
...
const fallbackRawPublicPage: PublicPageContent = {
  ...
  about: {
    ...
    body: {
      ar: "محتوى الصفحة يدار من لوحة التحكم.",
      en: "Public content is managed from dashboard.",
    },
  },
  restaurantPortal: {
...
export function languageLabel(language: string) {
  return (
    supportedLanguages.find((item) => item.code === language)?.nativeName ??
    language
  );
}
export async function getUiTranslations(language: string) {
  return fetchData<Record<string, string>>(
    `/translations/translations/${language}`,
    getRestaurantPageCopy(language),
  );
}

export async function getRestaurantPage(language: string) {
  return fetchData<Record<string, string>>(
    `/translations/restaurant-page/${language}`,
    getRestaurantPageCopy(language),
  );
}
