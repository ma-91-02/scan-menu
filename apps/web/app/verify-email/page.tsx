...
function VerifyEmailContent() {
  ...
  const [status, setStatus] = useState<string>(token ? copy.verifying : copy.required);
  ...
  return (
    <main className="auth-flow-page">
      <section className="public-form">
        ...
        {email ? (
          <button className="public-button primary" type="button" onClick={() => void resendVerification()}>
            {copy.resend}
          </button>
        ) : null}
        <a className="public-button secondary" href={`/?lang=${language}#login`}>{copy.back}</a>
      </section>
    </main>
  );
}
...
const verifyCopy = {
  ar: {
    ...
    required: "يجب تأكيد بريدك الإلكتروني قبل تسجيل الدخول. افحص بريدك أو أرسل رابطاً جديداً.",
  ...
  en: {
    ...
    required: "Please verify your email before signing in. Check your inbox or send a new link.",
  ...
  ru: {
    ...
    required: "Подтвердите email перед входом. Проверьте почту или отправьте новую ссылку.",
