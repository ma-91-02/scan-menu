...
import { buildPasswordResetEmailHtml, buildVerificationEmailHtml, getEmailCopy, getEmailCopyCoverage } from "./email.js";
...
test("email verification and password reset copy covers all supported languages", () => {
  ...
  assert.deepEqual(coverage.filter((item) => !item.ok), []);
