import { Pool } from "pg";
import type { LocalizedText, MenuItem, Order } from "@babili/shared";
...
export async function initOrderDatabase() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS babili_orders (
      id text PRIMARY KEY,
...
export async function getRestaurantLanguageDb(restaurantId: string) {
  const result = await pool!.query(
    "SELECT operating_language FROM babili_restaurants WHERE id = $1",
    [restaurantId],
  );
  return result.rows[0]?.operating_language
    ? String(result.rows[0].operating_language)
    : undefined;
}

export async function getMenuItemsDb(restaurantId: string) {
  const result = await pool!.query(
    "SELECT * FROM babili_menu_items WHERE restaurant_id = $1 AND is_available = true",
    [restaurantId],
  );
  return result.rows.map(mapMenuItem);
}

export async function getOrdersDb(query: {
  restaurantId?: string;
  customerId?: string;
}) {
  const result = await pool!.query(
    `SELECT * FROM babili_orders
...
export async function deleteOrdersByRestaurantDb(restaurantId: string) {
  await pool!.query("DELETE FROM babili_orders WHERE restaurant_id = $1", [
    restaurantId,
  ]);
}

export async function deleteOrdersByCustomerDb(customerId: string) {
  await pool!.query("DELETE FROM babili_orders WHERE customer_id = $1", [
    customerId,
  ]);
}
...
function mapOrder(row: Record<string, unknown>): Order {
  return {
    ...
    paymentMethod: row.payment_method
      ? (String(row.payment_method) as Order["paymentMethod"])
      : undefined,
    paymentStatus: row.payment_status
      ? (String(row.payment_status) as Order["paymentStatus"])
      : undefined,
